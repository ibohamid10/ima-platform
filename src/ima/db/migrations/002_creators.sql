CREATE TABLE IF NOT EXISTS creators (
    ...
    platform VARCHAR(32) NOT NULL CHECK (platform IN ('youtube', 'instagram', 'tiktok')),
    ...
    commercial_readiness_score INTEGER NULL CHECK (
        commercial_readiness_score IS NULL OR commercial_readiness_score BETWEEN 0 AND 100
    ),
    fraud_risk_score INTEGER NULL CHECK (
        fraud_risk_score IS NULL OR fraud_risk_score BETWEEN 0 AND 100
    ),
    evidence_coverage_score INTEGER NULL CHECK (
        evidence_coverage_score IS NULL OR evidence_coverage_score BETWEEN 0 AND 100
    ),
    consent_status VARCHAR(32) NOT NULL DEFAULT 'unknown' CHECK (
        consent_status IN ('unknown', 'legitimate_interest', 'consented', 'suppressed')
    ),
...
CREATE INDEX IF NOT EXISTS ix_creators_platform_handle ON creators (platform, handle);
CREATE INDEX IF NOT EXISTS ix_creators_qualified ON creators (is_qualified);
CREATE INDEX IF NOT EXISTS ix_creators_last_seen_at ON creators (last_seen_at);
